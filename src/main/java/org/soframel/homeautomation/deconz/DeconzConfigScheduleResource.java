...
public class DeconzConfigScheduleResource {
    ...
    @POST
    @Produces(MediaType.TEXT_HTML)
    public TemplateInstance saveSchedule(@QueryParam("thermostat") String thermostat, MultivaluedMap<String, String> form) throws Exception {
    ...
    private void addDataToSchedule(ScheduleForEachDay schedule, String key, String data) throws Exception{
        ...
        TransitionModel trans=switch (day) {
                ...
                case "saturnday" ->  this.getOrCreateTransition(schedule.getSaturdaySchedules(), index);
                case "sunday" ->  this.getOrCreateTransition(schedule.getSundaySchedules(), index);
                default -> throw new Exception("Unrecognized day in form data param"+key);
            };            
